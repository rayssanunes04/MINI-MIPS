...
// carregar arquivo .mem (instruções binárias)
void carregar_mem(struct simulador *sim) {
    ...
    // lê cada instrução binária linha por linha
...
void carregar_dados(struct simulador *sim) {
    ...
    // lê inteiros e coloca na memória
...
// salvar programa em formato .asm (binário simples)
...
void step_simulation(struct simulador *sim) {
    ...
    struct instrucao *inst = &sim->programa[sim->pc.pc]; // pega a minha instrução atual ( posição atual)
    ...
    printf("\nExecutando %d: %s\n", sim->pc.pc, inst->inst_char); // mostra qual a inst esta rodando
